    ...
    $.datePicker = {
        ...
        utils: {
            ...
            buildDecadePicker: function(century, year) {
                ...
                for (var i = 0; i < 3; i++) {
                    ...
                    for (var j = 0; j < 4; j++) {
                        ...
                        if (year >= num && year <= (num + 9)) { type += ' selected'; }
            ...
            buildCalendar: function(year, month, selected) {
                ...
                for (var w = 0; w < 7; w++) { days.append('<div class="cell">' + obj.strings.daysShort[w] + '</div>'); } calendar.append(days);
                for (var w = 0; w < 6; w++) {
                    ...
                    for (var d = 0; d < 7; d++) {
                        ...
                        if (numbering == date.getDate() && month == date.getMonth() && year == date.getFullYear()) { type += ' today'; }
                        if (numbering == selected.getDate() && month == selected.getMonth() && year == selected.getFullYear()) { type += ' selected'; } week.append('<a href="#" class="cell day' + type + '">' + num + '</a>');
                        numbering++;
                    }
        ...
        show: function(options) {
            ...
            var datePicker = null,
                date = new Date();
            if (opts.element) { if (typeof opts.element == 'string') { opts.element = $(opts.element); } date = opts.parseDate(opts.element.val()); }
            var selected = { day: date.getDate(), month: date.getMonth(), year: date.getFullYear(), decade: date.getFullYear() };
            ...
            if (opts.element) {
                ...
                datePicker.css({ left: offset.left + 'px', top: offset.top + opts.element.outerHeight(true) + 15 + 'px' });
            }
            datePicker.hide();
            ...
            datePicker.on('click', '.calendar .day', function(e) {
                ...
                if (el.hasClass('blank')) { return; } calendar.find('.selected').removeClass('selected');
                ...
                if (opts.selectDate(test)) {
                    ...
                    if (opts.closeOnPick && !el.hasClass('grayed')) { $.datePicker.hide(); }
                }
            });
            datePicker.on('click', '.calendar .month', function(e) {
                ...
                months = picker;
                calendar.fadeOut(150, function() { months.fadeIn(150); });
            });
            datePicker.on('click', '.calendar .prev', function(e) {
                ...
                if (el.hasClass('disabled')) { return; } month = month - 1;
                if (month < 0) {
                    month = 11;
                    year--;
                }
            ...
            datePicker.on('click', '.months .month', function(e) {
                ...
                var el = $(this),
                    ...
                    replacement = null;
                if (el.hasClass('blank')) { return; } months.find('.selected').removeClass('selected');
                el.addClass('selected');
                selected.month = month;
                replacement = $.datePicker.utils.buildCalendar(year, month, date);
                replacement.hide();
                calendar.replaceWith(replacement);
                months.fadeOut(150, function() { replacement.fadeIn(150); });
            });
            datePicker.on('click', '.months .prev', function(e) {
                ...
                var el = $(this),
                    ...
                    year = current.data('year');
                if (el.hasClass('disabled')) { return; } year -= 1;
                selected.year = year;
            ...
            datePicker.on('click', '.months .year', function(e) {
                ...
                years = picker;
                months.fadeOut(150, function() { years.fadeIn(150); });
            });
            datePicker.on('click', '.years .year', function(e) {
                ...
                var el = $(this),
                    ...
                    months = datePicker.children('.months'),
                    replacement = null;
                if (el.hasClass('blank')) { return; } else if (el.hasClass('next') || el.hasClass('prev')) { return; } years.find('.selected').removeClass('selected');
                el.addClass('selected');
                selected.year = year;
                ...
                years.fadeOut(150, function() { replacement.fadeIn(150); });
            });
            datePicker.on('click', '.years .prev', function(e) {
                ...
                if (el.hasClass('disabled')) { return; } decade -= 10;
            ...
            datePicker.on('click', '.years .decade', function(e) {
                ...
                years.fadeOut(150, function() { decades.fadeIn(150); });
            });
            datePicker.on('click', '.decades .decade', function(e) {
                ...
                var el = $(this),
                    ...
                    years = datePicker.children('.years'),
                    replacement = null;
                if (el.hasClass('blank')) { return; } else if (el.hasClass('next') || el.hasClass('prev')) { return; } decades.find('.selected').removeClass('selected');
                el.addClass('selected');
                replacement = $.datePicker.utils.buildYearPicker(decade, selected.year);
                replacement.hide();
                years.replaceWith(replacement);
                decades.fadeOut(150, function() { replacement.fadeIn(150); });
            });
            datePicker.on('click', '.decades .prev', function(e) {
                ...
                if (el.hasClass('disabled')) { return; } century -= 100;
            ...
            datePicker.on('click', '.decades .century', function(e) { e.preventDefault(); });
        ...
        hide: function(force) {
            var force = force || false,
                el = $('.datepicker');
            if (force) { el.remove(); } else { el.fadeOut(150, el.remove); }
        }
    };
    $.fn.datePicker = function(options) {
        ...
        this.each(function() {
            ...
            var callback = function(date) {
                ...
                if (locked.length) { for (var i = 0; i < locked.length; i++) { if (locked[i] == selected) { if (typeof $.alert === 'function') { $.alert = $.datePicker.strings.messageLocked; } else { alert($.datePicker.strings.messageLocked); } ret = false; break; } }; }
                return ret;
            };
            if (!button.length) { el.on('click', function() { $.datePicker.show({ element: el, selectDate: callback }); }); } else { button.on('click', function(e) { e.preventDefault(); if ($('.datepicker:visible').length) { $.datePicker.hide(); } else { $.datePicker.show({ element: el, selectDate: callback }); } }); }
        });
        return this;
    };
